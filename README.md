...
Open-source замена Outlook Web App, «Почты» Windows и мобильных почтовых
клиентов: один процесс FastAPI/uvicorn отдаёт почтовый интерфейс и
админ-панель поверх обычных IMAP/SMTP, без своего хранилища писем.

Работает и как self-hosted инсталляция на своём сервере, и как основа
...
Интерфейс — PWA: ставится на рабочий стол и на телефон, работает офлайн через
service worker, отдельного мобильного приложения не нужно.

Проект — форк рабочего веб-клиента, из которого убраны имена, адреса и
настройки конкретной инсталляции. Лицензия — MIT.
